...
def handle_query_responses(query_responses: list[tuple], analysis_plan: pd.DataFrame) -> tuple[dict, list]:
    '''
    ...
    for response in query_responses:
        ...
        if isinstance(response, Exception):
            logger.critical(f'Exception object returned while gathering asynchronous query results: {response}')
            logger.critical(f'Untrackable exception related to values-at-risk queries... exiting with code 1.')
            sys.exit(1)

        # unpack expected tuple structure
        identifier, var_alias, data = response

        # this determines which attributes get written to by batch writes
        analysis_types = write_analysis_types_dict(analysis_plan, var_alias)

        if isinstance(data, tuple) and issubclass(data[0], Exception):
            logger.error(json.dumps(
                {
                    'identifier': identifier,
                    'var_alias': var_alias,
                    'exception_info': format_logged_exception(*data)
                }
            ))
    ...
    return (query_features_dict, results_no_analysis)
