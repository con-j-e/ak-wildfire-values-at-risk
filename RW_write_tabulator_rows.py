...
def main():
    ...
    try:
        ...
        for tup in (
            ...
            ('buf_1', buf_1, deepcopy(nearest_feats_df)),
            ('buf_3', buf_3, deepcopy(nearest_feats_df)),
            ('buf_5', buf_5, deepcopy(nearest_feats_df))
        ):
            ...
            if buf_dist == 0:
                ...
                # nearest feats fields are already present for akdof_perims_locs, and must be dropped prior to join
                ...
                buf_dist_nearest_feats_df = buf_dist_nearest_feats_df.map(
                    lambda x: [feat for feat in x if not feat['dist_mi'] < 0] if x not in ('!error!','wfigs_IrwinID') else x,
                    na_action='ignore'
                )
                buf_dist_nearest_feats_df = buf_dist_nearest_feats_df.map(
                    lambda x: None if x == [] else x,
